...
PubHubs is a platform which is currently still looking for a suitable use case. Although there are many privacy
enhancing features present in PubHubs, many clients will not use it since it lacks features which are required for
their use case. We believe that having the ability to video call within PubHubs can be a very useful feature, which
for some clients is a necessity. This way, the privacy-friendly PubHubs can be used for numerous applications, like
online consultations, online classes, or just to have a chat with friends. However, currently there are no
organizations which are using PubHubs for this. So it is hard to say what the exact requirements are
...
\section{Meeting Core Requirements}
In this thesis we have set some core requirements. The first requirements where the CIA triad, which stands for
Confidentiality, Integrity, and Availability. We have achieved confidentiality by using end-to-end encryption
end-to-end encryption of video and audio streams using the AES-GCM encryption standard through Livekit's e2ee worker.
This ensures that even the SFU server cannot access the unencrypted content of calls. However, the current key
distribution mechanism, which relies on sending keys as clear-text Matrix room messages, prevents us for having a
perfect forward secrecy. While this is a temporary constraint due to Matrix's transitioning encryption library, it
represents a compromise in the current implementation. Albeit not a critical issue since it still requires you to
be in the room where the call occurs, it is a potential area for future improvement.

The solution also maintains data integrity through authenticated encryption, ensuring that streams cannot be tampered
with during transit. Since we use an SFU architecture together with end-to-end encryption, the server cannot modify
the encrypted content. This achieves the goal of protecting against "honest but curious" servers, since all the keys
are only available to the clients. However there is currently one tradeoff, the hub server knows the livekit token
of the user. This hub server could potentially use this token to join the call. This would be a problem if the hub
server is compromised, however the hub server would not receive the encryption keys and thus would not be able to
encrypt and decrypt the video data.
\todo{Is this really the case? Does authenticated encryption ensure integrity?}

Availability is another benefit of using an SFU architecture. The ability to deploy Livekit on separate servers and
scale horizontally means that the solution can handle a large number of participants without compromising the
functionality of the rest of PubHubs. This is however up to the hub to decide and configure, which should be made very
clear to the hub administrators. The hub administrators should calculate what an acceptable number of participants is
and how much resources they are willing to allocate to the video conferencing solution. Currently this is a rather technical
process, which could be made easier in the future by providing some guidelines or even an automatic scaling solution.

\todo{Dit hoofdstuk iets anders aanpakken}
When it comes to authentication, the solution uses the same authentication mechanism as the rest of PubHubs. A user would
need to authenticate with the hub server before they receive a token for the livekit server. Currently we only check if
the user is allowed to join the room, however we can also require the user to disclose certain YIVI attributes. However we
argue that this is not necessary since you can have a similar result by only allowing video conferencing in secured rooms.
You would then set up these secured rooms in such a way that the users have to disclose certain attributes. We believe that
this is fine in most cases, since it would not make sense to have a group call in a public room where you want to ensure that the
participants have certain attributes.

When it comes to who knows what, the server knows that a call is started and who is in the call. It actaully does not
keep track of who is in the call, but it can see who requested to join the video call. The server does not know the
encryption keys, so it cannot decrypt the video and audio data. The livekit server only knows the local pseudonym of the
users, which is already known by the hub server.
